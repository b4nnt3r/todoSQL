...
INSERT INTO todo (id, title, details, priority, created_at, completed_at) VALUES ('4', 'Buy a new t-shirt', 'size large', '3', '2017-08-05', '2017-08-06');
...
SELECT * FROM todo WHERE priority = 3 and completed_at is null GROUP BY created_at, todo.id;

SELECT COUNT(title) FROM todo WHERE completed_at is null GROUP BY priority, todo.id;

SELECT COUNT(created_at) FROM todo WHERE created_at > '2017-07-10' GROUP BY priority, todo.id;
